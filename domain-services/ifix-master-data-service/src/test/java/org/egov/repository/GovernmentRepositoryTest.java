 ...
 * 
 * @SpringBootTest() class GovernmentRepositoryTest {
 * 
 * @Autowired private TestDataFormatter testDataFormatter;
 * 
 * @Mock private MongoTemplate mongoTemplate;
 * 
 * @InjectMocks private GovernmentRepository governmentRepository;
 * 
 * private GovernmentRequest governmentCreateRequest; private
 * GovernmentSearchRequest governmentSearchRequest; private GovernmentResponse
 * governmentSearchResponse;
 * 
 * private ArrayList<String> idList;
 * 
 * @BeforeAll public void init() throws IOException { idList = new
 * ArrayList<>(); idList.add("GOVERNME-NT0B-CDEF-FEDC-ENTITY0UID10");
 * 
 ...
 * governmentRepository.findById(governmentCreateRequest.getGovernment().getId()
 * );
 * 
 * assertSame(governmentCreateRequest.getGovernment(), actualGovernment); }
 * 
 * @Test void testFindAllByIdList() { Query query = new Query();
 * query.addCriteria(Criteria.where("id").in(idList));
 * 
 * doReturn(governmentSearchResponse.getGovernment()).when(mongoTemplate).find(
 * query, Government.class);
 * 
 * List<Government> actualGovernmentList =
 * governmentRepository.findAllByIdList(idList);
 * 
